...
import pygame
# G: no freetype! see the example game "penguins cant fly"
# import pygame.freetype
from superduperfont import Font
...
def render_white_text_alpha_black_bg(font: Font,
...
class FontResource:
    ...
    def __init__(self,
        ...
        self.loaded_font = None  # type: Union[Font, None]
    ...
    def load(self):
        ...
        if isinstance(self.location, PackageResource):
            try:
                print('pygame-gui load font from bytes io package resource', str(self.location.resource))
            ...
            try:
                print('cwd = ' + os.getcwd())
                print('pygame-gui load font from location', self.location)
            ...
            try:
                print('pygame-gui load font from base64', self.location)
                file_obj = io.BytesIO(base64.standard_b64decode(self.location))
                self.loaded_font = Font(file_obj, self.size, resolution=72)
                self.loaded_font.pad = True
                self.loaded_font.origin = True
                if self.force_style:
                    self.loaded_font.strong = self.style['bold']
